...
node_exporter, cAdvisor, postgres_exporter, Uptime Kuma, and GlitchTip.
Monitors the whole host - both sites (zvonok, wallet) behind the shared
Traefik gateway. Research and decision rationale:
...
```
VPS (4 vCPU / 4 GB)
  prometheus (host net, :9090) --scrapes--> 127.0.0.1:{3000 app, 9100 node,
      8080 cadvisor, 9187 postgres, 8082 traefik}
  node_exporter (host net, :9100)
  postgres_exporter (host net, :9187 -> 127.0.0.1:5432)
  cadvisor (:8080, loopback publish)
  grafana        -- web network --> grafana.yurifa.site  (datasource:
                    host.docker.internal:9090)
  uptime-kuma    -- web network --> status.yurifa.site
  glitchtip(+worker) -- web + zvonok networks --> errors.yurifa.site,
...
2. **Firewall**: keep monitoring ports off the internet. If ufw is active
   ...
   enable ufw first.
3. **Gateway edit** (`~/gateway/docker-compose.yml`). Traefik exposes
   metrics on an entrypoint - without a dedicated one they would land on
   the public `web` entrypoint. Add a metrics entrypoint bound to the
   container, publish it on host loopback, and switch metrics to it:
...
4. **Database roles**: replace the passwords in
   `monitoring/sql/monitoring-init.sql`, then run it inside the VPS
   postgres container (name and user from `docker exec zvonok-postgres env
   | grep POSTGRES`):
   `docker exec -i zvonok-postgres psql -U zvonok_admin -d zvonok < monitoring/sql/monitoring-init.sql`
5. **DNS**: A records for `grafana.` / `status.` / `errors.yurifa.site`
   pointing at the VPS IP (Traefik issues LE certs on first request).
6. **Deploy**:
   `scp -r monitoring/ vps:~/monitoring`, then on the VPS:
...
Docker community dashboards). Alert rules: `provisioning/alerting/`.
